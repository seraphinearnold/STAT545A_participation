...
```{r}
fox %>%   #start with a fox senctence -> one vector
  str_split(" ") %>% #split every character(=letter) with nothing(because Leertaste)
  `[[`(1) %>% #a function: a list as a train of cars, componence of the car. In this case componence of the first car.
    str_to_lower() %>%   #converts to string characters
  table()
```
...
```{r}
fox %>% 
    str_replace(pattern="fox", replacement="giraffe") #which pattern is ist that should be replaced?-> fox. With what should it be replayced?-> giraffe
```
...
```{r}
str_c(words[1:4], words[5:8], sep=" & ") #taking the first 4 words of the words data and combine them with the next 4 words of the words data
str_c(words[3:4], words[5:8], sep=" & ") #about and absolute recycled 
...
```{r}
contents <- str_c("# Participation\n\n Participation for class meeting ", 1:12)  #backslash= new line. n=press enter
```
