...
```{r}
write_csv(iris, "iris.csv")  #this will print out a csv file of my data frame
```

Write the `iris` data frame to a file delimited by a dollar sign. 

```{r}
write_delim(iris, "iris.txt", delim = "$")  #$ als Trennung 
```
...
read.csv("https://github.com/jennybc/lotr-tidy/blob/master/data/The_Return_Of_The_King.csv")
```
