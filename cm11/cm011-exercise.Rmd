...
```{r}
course[1]
course[-1]   #is how to remove the entry above
sort(course)[1]
```
...
10:-5     # negative vector from 10 to -5
1:0       # loop
seq_len(0)#Vector of length O
seq_len(10)
```
...
```{r}
a ^ 2    # entire vector mit allen components im Quadrat
```

Multiply each component by 1 through its length:

```{r}
a * 1:10    # erstes Objekt mal 1, zweites Objekt mal 3, usw.
```
...
```{r}
a * 1:3     # Reicht nur bis zum dritten Objekt -> R recyclet den Vektor und macht beim vierten Objekt wieder mal 1, beim fünften 2, usw. -> Aufpassen, wenn man das nicht intendiert
```
...
a < 5
a > 0 & a < 5
a > 0 && a < 5  # not vectorized (used for if statements)
...
```{r}
setNames(course, c("subject", "course", "section")) # doesn`t change the course object itself
course
```
...
```{r}
names(course) <- c("subject", "course", "section") # changes the actual course vector
```

3. Re-constructing the vector, specifying names within `c()`.

```{r}
course <- c(subject="STAT", code="545A", section="S01") #overwrites the actual vector
course
```
...
(a <- 7:-2)
(n <- length(a))
a[a > 0]  # omits all entries that are less than 0 
a[a !=4]  # get rid of the 4
```
...
# List is like a train, the different wagons can contain different types of things
```
...
```{r}
my_list[2] # subset the list to it's second wagon/component
my_list[[2]] # don't give me the second component, but what's in it/it's contence
```
...
```{r}
as.list(small_df)   # are in their own columnswithout interfering with each other
```
...
identical(list(4), 4) 
identical(c(4), 4)
# check if the objects are the same: lists do have certain things around them, vectors not
```
