...
```
```{r}
map2(a, b, `*`)  #aterix function applied to a and b
```
```{r}
map2_dbl(a, b, `*`)  #in vector form - because the funtion we're apllying might not be vectorized (in this case it was though)
```
...
```{r}
5 + 7
`+` (5, 7)   #same meaning, plus as a function
```



More than 2 functions? Use the `pmap` family:
...
```
```{r}
pmap_dbl(list(a, b, c), ~ ..1 * ..2 * ..3)  #dbl = double mapping
```
...
```{r}
str_c(name, " takes ", commute, " minutes to get to work.") #output we'er looking for
```


1. By defining a function before feeding it into `map2_chr()` -- call it `comm_fun`.

```{r}
comm_fun <- function(s, t)
  str_c(name, " takes ", commute, " minutes to get to work.")  #defined function
...
```{r}
#got_chars   #got = game of thrones
             #we want a tibble with the first entry of name and than aliases
got_alias <- got_chars %>% {
...
```{r}
icecream %>% 
  filter(map_lgl(flav, ~ FILL_THIS_IN))  #map_lgl = map logical / flav is a list of 3
```

```{r}
icecream %>% 
  filter(map_lgl(flav, ~ any(.x == "strawberry"))) # x is icecream and we want any to be equal to strawberry 
```
...
str(got_alias)
got_alias %>% 
  drop_na() %>% #to get rid of the list with NULL
  unnest()      #now we can unnest
```
...
```{r}
iris %>% 
  group_by(Species) %>%  #does the opposite than the function before
  nest()
```
