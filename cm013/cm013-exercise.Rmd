...
- Use `ggsave` for ggplot
    - Practice by saving the following plot to file: 
     #when you want to safe just a plot itself, e. g. as a pdf:
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10) #FILL_IN_BREAKS indicates what numbers to put tick marks
    ) +
    scale_y_continuous(breaks = 1 : 10 * 10)     #FILL_IN_SCALE_FUNCTION(breaks=FILL_IN_BREAKS)
```
...
4. Check out code to make a plotly object from scratch using `plot_ly()` -- scatterplot of gdpPercap vs lifeExp.
    ...
    #this is how to make a Plot from scratch
